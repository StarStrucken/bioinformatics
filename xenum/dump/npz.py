...
def write_npz(out_dir, measurement, graph_nodes, edges):
    ...
    edge_columns = ["neighbor_distance", "neighbor_weight", "xy_distance", "expression_distance", "morphology_distance", "same_component"]
    edge_attr = edges[edge_columns].to_numpy(dtype=np.float32) if len(edges) else np.empty((0, len(edge_columns)), dtype=np.float32)
    np.savez_compressed(
        ...
        edge_feature_columns=np.asarray(edge_columns, dtype=str),
