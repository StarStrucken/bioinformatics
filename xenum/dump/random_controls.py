...
def prediction_seed_path(out_dir, measurement, seed, k):
    return out_dir / f"predictions_{measurement}_seed{int(seed)}_k{int(k)}.csv"

def nodes_seed_path(out_dir, measurement, seed, k):
    return out_dir / f"nodes_{measurement}_seed{int(seed)}_k{int(k)}.csv"

def edges_seed_path(out_dir, measurement, seed, k):
    return out_dir / f"edges_{measurement}_seed{int(seed)}_k{int(k)}.csv"

def checks_seed_path(out_dir, measurement, seed, k):
    return out_dir / f"checks_{measurement}_seed{int(seed)}_k{int(k)}.json"

def copy_selected_prediction(out_dir, measurement, seed, k):
    path = prediction_seed_path(out_dir, measurement, seed, k)

    if path.exists():
        df = pd.read_csv(path)
        df.to_csv(out_dir / f"predictions_{measurement}_k{int(k)}.csv", index=False)

def copy_selected_graph(out_dir, measurement, seed, k):
    pred_path = prediction_seed_path(out_dir, measurement, seed, k)
    node_path = nodes_seed_path(out_dir, measurement, seed, k)
    edge_path = edges_seed_path(out_dir, measurement, seed, k)
    check_path = checks_seed_path(out_dir, measurement, seed, k)

    if pred_path.exists():
        pd.read_csv(pred_path).to_csv(out_dir / f"predictions_{measurement}_k{int(k)}.csv", index=False)

    if node_path.exists():
        pd.read_csv(node_path).to_csv(out_dir / f"nodes_{measurement}_k{int(k)}.csv", index=False)

    if edge_path.exists():
        pd.read_csv(edge_path).to_csv(out_dir / f"edges_{measurement}_k{int(k)}.csv", index=False)

    if check_path.exists():
        (out_dir / f"checks_{measurement}_k{int(k)}.json").write_text(check_path.read_text())
...
def run_random_permutation(out_dir, dataset_id, nodes):
    ...
    for seed in tqdm(RANDOM_SEEDS, desc=RANDOM_PERMUTATION):
        ...
        rows.append(row)
        prediction_seed_path(out_dir, RANDOM_PERMUTATION, seed, 0).parent.mkdir(parents=True, exist_ok=True)
        pred_df.to_csv(prediction_seed_path(out_dir, RANDOM_PERMUTATION, seed, 0), index=False)

    best = selected_row(rows)

    if best is not None:
        copy_selected_prediction(out_dir, RANDOM_PERMUTATION, best["seed"], 0)
...
def run_random_neighbors(out_dir, dataset_id, nodes, blocks, bench_k_values):
    ...
    for seed in tqdm(RANDOM_SEEDS, desc=RANDOM_NEIGHBORS):
        for kk in bench_k_values:
            ...
            graph_nodes.to_csv(nodes_seed_path(out_dir, RANDOM_NEIGHBORS, seed, kk), index=False)
            edges.to_csv(edges_seed_path(out_dir, RANDOM_NEIGHBORS, seed, kk), index=False)
            pred_df.to_csv(prediction_seed_path(out_dir, RANDOM_NEIGHBORS, seed, kk), index=False)
            checks_seed_path(out_dir, RANDOM_NEIGHBORS, seed, kk).write_text(json.dumps(chk, indent=2) + "\n")
    ...
    if best is not None:
        ...
        best_k = int(best["k"])
        copy_selected_graph(out_dir, RANDOM_NEIGHBORS, best_seed, best_k)
        graph_nodes = pd.read_csv(out_dir / f"nodes_{RANDOM_NEIGHBORS}_k{best_k}.csv")
        edges = pd.read_csv(out_dir / f"edges_{RANDOM_NEIGHBORS}_k{best_k}.csv")
        write_npz(out_dir, RANDOM_NEIGHBORS, graph_nodes, edges)
        summaries[RANDOM_NEIGHBORS] = json.loads((out_dir / f"checks_{RANDOM_NEIGHBORS}_k{best_k}.json").read_text())
